          ...
          <div className="space-y-6">
            <div className="info-container">
              <div>
                {' '}
                Elevation:{' '}
                <span className="highlight">{modelcovalue?.elevation}</span>
              </div>
              <div>
                latitude -coordinate{' '}
                <span className="highlight">
                  {modelcovalue?.coordinate?.lat}
                </span>
              </div>
              <div>
                {' '}
                ...
                <span className="highlight">
                  {modelcovalue?.coordinate?.lng}
                </span>
              </div>
              <div>
                {' '}
                place: <span className="highlight"> {locationName}</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <div className="space-y-6">
                {/* Buttons */}
                <div
                  onClick={() => handleButtonClick('Modified Free Space')}
                  className="w-full"
                >
                  <Button>Modified Free Space EBS</Button>
                </div>
                <div
                  onClick={() => handleButtonClick('Egli Model EBS')}
                  className="w-full"
                >
                  <Button>Egli Model EBS</Button>
                </div>
                <div
                  onClick={() => handleButtonClick('Modified Hata Model ITV')}
                  className="w-full"
                >
                ...
                <div
                  onClick={() => handleButtonClick('Egli Model ITV')}
                  className="w-full"
                >
                  <Button>Egli Model ITV</Button>
                </div>
                <div
                  onClick={() => handleButtonClick('Hata Model STV')}
                  className="w-full"
                >
                  <Button>Modified Hata Model for STV</Button>
                </div>
                <div
                  onClick={() => handleButtonClick('Egli Model STV')}
                  className="w-full"
                >
              ...
              >
                ...
                {selectedButton === 'Modified Free Space' && (
                  <ModifiedFreeSpace />
                )}
                {selectedButton === 'Modified Hata Model ITV' && (
                  <ModifiedHataModelITV />
                )}
