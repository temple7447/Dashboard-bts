...
function EvationLocation() {
  ...
  const HandleScanSuitableHeight = () => {
    const retrievedData =
      JSON.parse(localStorage.getItem('suitableHeight')) || [];

    const result = retrievedData.reduce(
      (acc, current) => {
        ...
        if (currentWeightUtility > acc.weightutlity) {
          ...
          currentWeightUtility === acc.weightutlity &&
          currentElevation > acc.elevation
        ) {
    ...
    ?.map((item, index) => {
      return (
        <div key={index} className="" style={{ gap: 10 }}>
          <div>
            {' '}
            Name of Place Scanned:
            <span className="highlight"> {item.Name}</span>{' '}
          </div>
          <div>
            Population in the Area:{' '}
            <span className="highlight"> {item.population}</span>{' '}
          </div>
        </div>
      );
    });
  const modealInfo = population?.filter(item => {
  ...
  const {
    coordinate: suitableCoordinate,
    elevation: suitableelevation,
    weightutlity: suitableWeight,
  } = suitable;
      ...
      {topElevationss.length > 0 ? (
        <div>
          <button
            ...
            className="mx-2 my-4"
            onClick={() => {
              HandleScanSuitableHeight();
              toast.success('Scanning for Suitable Site', {
                position: 'top-right',
                autoClose: 5000,
          ...
          <button
            ...
            className="mx-2 my-4"
            onClick={() => {
              setScanAgain(pre => !pre);
              localStorage.removeItem('suitableHeight');
              toast.success('Previous Suitable Site has been clear', {
                position: 'top-right',
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
          ...
          <Table striped>
            <Table.Head>
              ...
              <Table.HeadCell>Longitude Value</Table.HeadCell>
              <Table.HeadCell>Latitude Value</Table.HeadCell>
              <Table.HeadCell>Elevation Value</Table.HeadCell>
                  ...
                  >
                    ...
                    <Table.Cell>
                      ...
                      <Button
                        onClick={() => {
                          calculatePathLoss();
                          HandleShowModel(item, locationName, modealInfo);
                        }}
                      >
                        Empirical model
                      </Button>
