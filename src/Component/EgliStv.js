...
const EgliStv = () => {
  ...
  const { useDistace, setdistance, setglobarpathloss, mute, setMute } =
    useInformation();
  ...
  const calculateLogarithm = () => {
    const Dkm = parseFloat(distance);
    const ht = 72.2; // in meters
    const hr = 1.5; // in meters

    if (!isNaN(Dkm)) {
      const logResult =
        40 * Math.log10(Dkm) - 20 * Math.log10(ht) - 10 * Math.log10(hr);
      setResult(logResult);
  ...
  return (
    <div className="egli-model-container">
      ...
      <div className="info-text">
        Transmitter Height = 76.2m and Receiver Height = 1.5
      </div>
      <label>
      ...
      <button
        className="calculate-button my-4 mx-2"
      ...
      {result !== null && (
        <div className="result-container">
          {' '}
          {/* Apply CSS class to the result container */}
