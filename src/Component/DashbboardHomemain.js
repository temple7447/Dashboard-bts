    ...
    fetch(apiUrl)
      ...
      .then(data => {
        if (data.results && data.results.length > 0) {
          ...
          console.log(
            `Coordinates for "${locationName}": Latitude ${lat}, Longitude ${lng}`
          );
          setlatgeo(lat);
          setlonggeo(lng);
    ...
    <div className="bodyimage" style={{}}>
      ...
      >
        {alertmesuca && (
          <Alert
            ...
            color="success"
            onDismiss={handleSuccessDismissa}
          >
            <span>
              <p>
        ...
        <div>
          {' '}
          <Avatar alt="avatar of Jese" img={imageimage} rounded />
        </div>
      </div>
      ...
      <div className="gap-2 flex flex-row justify-around my-5">
        <div className="info-container mx-10">
          <div>
            Total Coordinate Scanned:{' '}
            <span className="highlight">
              {scannedCoordinates.length}
              <>
                <div style={{ display: 'flex', flexDirection: 'row', gap: 10 }}>
                  <Button onClick={() => handleScannedCoordinates()}>
                    Scan coordinates
                  </Button>
                </div>
                <Modal
          ...
          <div>
            Number of Altitude to be Picked:{' '}
            <span className="highlight">{numTopAltitudes}</span>
          </div>
          <div>
            Coverage Radius (Kms):{' '}
            <span className="highlight">{useDistace}</span>
          </div>
          <div>
            Bandwidth Required : <span className="highlight">0.15</span>
          </div>
        </div>
        <div
          className="p-3 "
      ...
      <div className="flex flex-row">
        <MapChart
          northeastp={northeastp}
          southwestp={southwestp}
          shatterbar={shatterbar}
        />
        <Tables />
      </div>
    </div>
